...
latex <- function(
  ...
  reg.df <- .coef_df(summs[[1L]], n.digits, big.mark, "latex")
  colnames(reg.df) <- c("Predictor", model.names[1L])

  for (i in seq_along(summs)[-1L]) {
    temp.df <- .coef_df(summs[[i]], n.digits, big.mark, "latex")
      ...
      sapply(
        ...
        function(model) {
          vapply(
            ...
            function(stat) {
              if (stat != "nobs") {
                .fmt(do.call(.get_fun(stat), list(model)), n.digits, big.mark)
              } else {
                as.character(do.call(.get_fun(stat), list(model)))
              }
            },
            character(1L)
          )
      ...
      matrix(
        vapply(
          ...
          function(model) {
            if (stats.ncv != "nobs") {
              .fmt(
                do.call(.get_fun(stats.ncv), list(model)),
                n.digits,
                big.mark
              )
            } else {
              as.character(do.call(.get_fun(stats.ncv), list(model)))
            }
          },
          character(1L)
        ),
...
html <- function(models, n.digits, big.mark, caption, spacing, stats, cv.args) {
  ...
  summs <- lapply(models, summary)
  reg.df <- .coef_df(summs[[1L]], n.digits, big.mark, "html")
  colnames(reg.df) <- c("Predictor", model.names[1L])

  for (i in seq_along(summs)[-1L]) {
    temp.df <- .coef_df(summs[[i]], n.digits, big.mark, "html")
    colnames(temp.df) <- c("Predictor", model.names[i])
    reg.df <- merge(reg.df, temp.df, by = "Predictor", all = TRUE, sort = FALSE)
  }
...
reg.table <- function(
  ...
  if (
    "K.vals" %in% (cv.argnames <- names(cv.args)) && length(cv.args$K.vals) > 1L
  ) {
    stop("Argument 'K.vals' must be a single integer value.", call. = FALSE)
  }

  if (is.null(cv.argnames) && length(cv.args[[2L]]) > 1L) {
    stop("Argument 'K.vals' must be a single integer value.", call. = FALSE)
  }

  regression.tables <- vapply(
    model.chunks,
    TBL.FUN,
    character(1L),
    n.digits = n.digits,
